...
**PROD-004 (P1): matter soft-delete does NOT cascade to documents/citations/timeline — orphaned "zombie" data** | Status: OPEN (found 2026-06-10 during E2E test-matter cleanup)
- **Symptom.** Deleting matters via the dashboard (bulk "Delete Selected") sets `matters.deleted_at` and the confirm dialog states *"This will delete all documents, citations, and timeline events associated with these matters"* — but **the documents keep `deleted_at=NULL`, `status='completed'`** and citations stay live. Live: 3 just-deleted matters → **0/3 documents soft-deleted, 36 citations still live** under them. System-wide: all soft-deleted matters show 0 cascaded docs.
- **Impact.** (a) UI promise ≠ behavior. (b) **Zombie data**: any consumer that reads documents/citations WITHOUT joining `matters.deleted_at` sees deleted matters' data as live — including the **system_invariants watchmen** (`verified_citation_vintage_mismatch`, `verified_section_token_mismatch`), which therefore over-count by including deleted-matter citations. (c) Storage/cost not reclaimed.
- **Assumption corrected (honest).** I initially flagged `matters.status` staying `'active'` after delete as the INF-009 "soft-delete must update status" parallel-truth. **Live data disproved it**: `matters.status` is `'active'` on ALL 29 matters — it is NOT a liveness field here; `deleted_at` is the sole deletion flag. So the status field is a non-issue; the real bug is the missing cascade. (Example of assumption-from-memory vs actual site behavior.)
- **Fix direction (not yet done).** Either cascade `deleted_at` to documents+citations+timeline on matter delete (a "remember to cascade" rule — ARCH-003 risk), OR — better — make all reads filter through `matters.deleted_at` via join / a reconciler that derives liveness (so no cascade to remember). The watchmen should at minimum exclude `matters.deleted_at IS NOT NULL` citations. Needs blast-radius + choose-solution. Related: feedback-soft-delete-status memory, INF-009.
