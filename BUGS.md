...
**GAP-14 (P3): No delete path (soft OR hard) for library documents** — No way to remove a bad/garbage library act without direct DB access.
- **Confirmed codebase-wide 2026-06-10** (during PROD-005 end-to-end testing). `library_documents` has **no `deleted_at` column** (so no soft-delete) AND **no delete endpoint or service method** at all: the only library DELETE route is `library.py:353 unlink_library_document` → `LibraryService.unlink_from_matter`, which deletes the `matter_library_links` join row, **not** the `library_documents` row. Grep for `table("library_documents").delete` across `backend/app` = 0 matches. The act stays in the global library forever; failed/test/garbage acts can only be removed by a DBA.
- **Shape.** This is the asymmetric-lifecycle twin of PROD-005/PROD-004: `documents` got soft-delete + cascade + a 30-day hard-delete sweep (`hard_delete_expired_documents`), but the parallel `library_documents` table got an ingest path and never a delete path (GAP-8 parallel-tables divergence — every lifecycle capability added to `documents` must be remembered for `library_documents`, and delete was forgotten).
- **FK note (for any future fix or manual delete).** A hard `DELETE` of a `library_documents` row is FK-safe: `matter_library_links` + `library_chunks` are `ON DELETE CASCADE`; `act_resolutions.act_document_id`, `documents.library_document_id`, `citations.target_act_document_id`, `llm_costs.library_document_id` are all `ON DELETE SET NULL` — so a referenced act would silently null live pointers (verification/cost attribution). Any real delete path MUST check those 4 referrers first (and ideally orphan-clean the Supabase Storage blob at `storage_path`, which no cascade touches).
- **Instance handled manually 2026-06-10.** Hard-deleted `library_documents.id=97edad7f-…` (`Test_Unique_Act_2026.pdf`, status=failed) via raw SQL after confirming 0 referrers in the 4 SET-NULL tables; 1 `matter_library_links` row cascaded, 0 chunks. Its Storage blob was left orphaned (negligible, test data).
- **Fix direction (when prioritized).** Mirror the `documents` lifecycle on `library_documents`: add `deleted_at`, a soft-delete service method + admin/owner endpoint, and extend `hard_delete_expired_*` to reclaim them — i.e. close the GAP-8 divergence rather than bolt on a one-off hard-delete button. Still P3 (admin-only operational annoyance, no user-facing breakage).
...
| **Status** | **CORE FIX DEPLOYED + LIVE-VERIFIED (L2) — prod 2026-06-11** (commit `f196c6f`, Vercel → www.jaanch-ai.in) |
| **Date Found** | 2026-05-20 |

> **Live prod verification (2026-06-11, Playwright on www.jaanch-ai.in, authed):** bad matter `00000000-…` → clean **"Matter not available"** MatterErrorScreen, **shell never mounts** (no header/tabs/panels), **exactly 1 API call** (`GET /api/matters/0000…` → 404, the single gate fetch) and **1 console error** (that same 404) — down from ~18. Valid matter (`trial`) → full shell renders (header w/ real name, Summary/Timeline/Documents tabs), **0 console errors**. FE-003 confirmed dead end-to-end.
>
> **L2 fix built 2026-06-11.** chose-solution → L2 (client convergence gate); blast-radius Phase 2 swept the de-fabrication; architecture-guard answered; hostile-review found 1 bug (fixed) + accepted RISK 1. `tsc`+`eslint` clean.
...
> **FE-007 — RESOLVED 2026-06-11 (was deferred here).** Shipped as choose-solution **L2** (PR #66, branch `fix/fe-007-failed-matter-status`; backend `LDIP` + `ldip-worker` + Vercel all deployed). Root shape: matter status was derived in **3 disagreeing places**, none able to express failure (badge = processing/ready only; filter re-derived `needs_attention` from a `verificationPercent` mocked at 0 → matched every matter; only the in-workspace banner saw FAILED). **Fix:** backend `tab_stats_service._get_processing_status` now derives `'failed'` read-only from `documents.status` ∈ {ocr_failed,chunking_failed,embedding_failed,failed} (two reads degrade independently); a single `deriveMatterStatus()` now feeds badge + filter + counts (precedence processing>failed>needs_attention>ready) — killing the 3-way divergence AND the dead `'needs_attention'` value; distinct red **"Failed"** badge + "View matter" action; TS **exhaustiveness guard** (`never`) makes a future unhandled status a compile error. architecture-guard (read-only, anti-ARCH-003) + hostile-review (R1 independent-degradation fixed; R2 processing>failed precedence accepted). **LIVE-VERIFIED on prod**: matter `c8a71ed6` ("8 & 9 juhinebhnani4", 1 failed doc) shows the red Failed badge + copy + Retry re-queues the doc; healthy matters still Ready/Needs Attention; no false Failed. FE-010 (custom 404) and FE-011 (stuck summary spinner) remain separate/open.
...
|-------|-------|
| **Severity** | P1 (Architectural — frontend) |
| **Status** | **MATTER SHELL + ALL-TAB CONTENT REFLOW FIXED (L2) + DEPLOYED + GENUINELY LIVE-VERIFIED (eyes-on-pixels) 2026-06-12** (debt narrowed, not fully closed; see Resolution + Resolution Part 2) |
| **Date Found** | 2026-05-20 |
| **Source** | Frontend audit + 4-agent code census (2026-05-20). Evidence: `FRONTEND-AUDIT-2026-05-20.md` §3. |
...
**Resolution Part 2 — full responsive pass + honest correction (2026-06-12)**: the 2026-06-11 shell fix above was marked "LIVE-VERIFIED" off the Playwright **accessibility tree**, which is NOT visual verification — on real pixels it rendered poorly (full-screen sheet takeover, doubled headers, summary content clipped, raw scrollbar troughs). That mistake produced new guardrails (CLAUDE.md "Verify UI Visually — Pixels Are the Gate"; `visual-verify` skill; hostile-review Post-Deploy step 5; GUARDRAIL-BACKLOG **B4.5** = CI Playwright visual regression; memory `feedback-visual-is-the-gate` / `-agent-gate-discipline` / `-fanout-blind-cross-surface`). The full pass:
- **Foundation**: Q&A split now collapses at **`lg` (1024)** not `md` (fixes the cramped iPad-portrait split); `no-scrollbar` utility added (cascades to descendants, so it reaches `ui/table.tsx`'s own overflow wrapper); `QAPanel` gained a `compact` prop so the mobile bottom-sheet hides desktop-only model selectors + the duplicate header.
- **New shared primitive `frontend/src/components/ui/responsive-table.tsx`** — ONE DOM tree, each column's `cell()` rendered by BOTH the desktop table and the mobile card (kills the table-vs-card duplicate-logic shape that a first multi-agent attempt had reproduced); select-all + per-row `disabled` in both modes. **DocumentList, VerificationQueue, CitationsList (flat path)** converted onto it — the 3 wide tables that only scrolled sideways on mobile now become stacked cards. CitationsList grouped tree kept as a clean `no-scrollbar` horizontal scroll (hierarchy doesn't fit a flat primitive — chosen knowingly).
- **Content reflow across all 7 tabs + dashboard**: summary parties/cards stack, timeline header stacks with icon-only toggles, contradictions statements stack, dashboard QuickStats become vertical cards (labels wrap, no "Acti…"), entity merge-suggestion rows wrap (no one-word-per-line), citations sidebar/split-view collapse below `lg`.
- **Verified by failing-test-NAME diff vs master (not counts): 0 new failures across all touched dirs (18 pre-existing citation/verification/document failures fixed); `tsc` + full `eslint src` clean.** (One regression — VerificationQueue checkbox-disabled — was *caught by the name-diff* and fixed; a count-only check had hidden it.)
- **GENUINE visual verification (eyes on pixels) at 375/768/1024/1280 on every surface** found + fixed 3 live defects: scrollbar troughs → `no-scrollbar` cascade; dashboard label truncation → vertical cards; entity one-word-per-line → wrap.
- **Still deferred**: the ESLint structural-breakpoint wall (B4.5/B4.x), CitationsList grouped→cards, the ~62 benign cosmetic breakpoint usages, and `EntitiesGraph`/`LinkedLibraryPanel` responsive (flagged unaudited).
...
|----|-----|--------|-------|--------|
| **FE-001** | P1 | **FIXED (L2) + PIXEL-VERIFIED 2026-06-12** | "Ask jaanch" panel never collapses on mobile → collapses to a bottom-sheet drawer below **lg (1024)** via useBreakpoint. Eyes-on-pixels verified @390 + @768 (split gone, bottom trigger opens sheet) and @1280 (split intact). (2026-06-11 "verified" was off the DOM tree only — see FE-ARCH-02 Resolution Part 2.) | FE-ARCH-02 |
| **FE-002** | P1 | **FIXED (L2) + PIXEL-VERIFIED 2026-06-12** | Matter header buttons clipped at 390 → "Dashboard" label hidden below sm + min-w-0 center; pixel-verified actions no longer clip @390 | FE-ARCH-02 |
...
| **FE-004** | P2 | **FIXED (L2) + PIXEL-VERIFIED 2026-06-12** | Dashboard stats overflowed + labels truncated ("Active Ma…") → MobileQuickStats is now a 3-col grid of vertical cards; labels wrap fully, no horizontal scroll/trough. Pixel-verified @390 | FE-ARCH-02 |
| **FE-005** | P2 | **FIXED (L2) + PIXEL-VERIFIED 2026-06-12** | "Documents" tab label cut off → tab strip `overflow-x-auto no-scrollbar`; pixel-verified @390 | FE-ARCH-02 |
| **FE-006** | P2 | **FIXED (L2) + PIXEL-VERIFIED 2026-06-12** | Verification/Documents/Citations tables clipped/sideways-scrolled on mobile → now stacked cards below lg via the shared `responsive-table` primitive. Pixel-verified @390 with real data (verification cards: status + Approve/Reject/Flag; documents cards; citations flat cards). Citations *grouped* tree still horizontal-scrolls (clean, no trough) — deferred | FE-ARCH-02 (was: compounds with FE-001) |
...
**PROD-005 (P2): `soft_delete_document` writes `status='deleted'`, REJECTED by the live CHECK constraint → single-document delete 500s** | Status: **FIXED + DEPLOYED + E2E-VERIFIED 2026-06-10** (backend commit `1484b75`, LDIP `f557121f` + ldip-worker `eb07d0f7` both SUCCESS; frontend label `8c1ae62` on Vercel prod; migration `20260610000001` live; matter-delete E2E proven on prod). Folds in PROD-004 Part A.
...
- **Frontend `'deleted'` label — SHIPPED 2026-06-10 (commit `8c1ae62`, deployed to Vercel prod, aliased www.jaanch-ai.in).** Added `'deleted'` to the `DocumentStatus` union (`frontend/src/types/document.ts`) + a `'Deleted'` entry to `STATUS_LABELS` (`DocumentList.tsx`); badge color falls through to the muted-gray default (correct for a removed item, not red-error). `tsc --noEmit` clean (no exhaustiveness breaks). Defensive only — **unreachable via UI**: every document-list endpoint filters `deleted_at IS NULL`, so a `'deleted'` doc never reaches a list; only a direct detail fetch of a deleted doc could surface it. Related: feedback-soft-delete-status memory, INF-009, GAP-8.
- **E2E production-verified 2026-06-10.** Ran the real deployed `delete_matter` against prod for matter `774ea10f` (Shiju K vs Nalini): matter got `deleted_at`, its document `5b4d03f2` got `status='deleted'`+`deleted_at` (same timestamp) — confirming both the constraint fix (PROD-005) and Part A stamping (PROD-004) work end-to-end. `soft_delete_document` constraint acceptance separately proven via a `BEGIN;UPDATE…ROLLBACK;`.

**PROD-006 (P1): password reset / magic-link login / signup confirmation all silently throttled — Supabase built-in email sender is capped at 2 emails/hour (locked until custom SMTP)** | Status: OPEN (root cause confirmed live 2026-06-11; fix is a dashboard/config action by the owner)
- **Symptom.** Password reset "loops": clicking the reset link returns to `/forgot-password` with no message. Magic-link login shows "email rate limit exceeded." New-user signup confirmation emails likely also fail.
- **Root cause (confirmed from live Supabase auth logs + dashboard, project `xmbtcgmjvdouqstiqqom`).** The project uses Supabase's **built-in email service**, whose **email send rate limit is 2/hour** (Authentication → Rate Limits, field is **greyed/locked** — per Supabase docs it only becomes editable once **custom SMTP** is configured). All email-based auth (recovery, magic link, signup confirm) shares that one 2/hour bucket. Logs showed `POST /recover → 429 over_email_send_rate_limit` and `auth.users.recovery_sent_at` frozen weeks back (no fresh email sent), so users click a stale link → `GET /verify → 403 otp_expired`. Per Supabase docs the built-in sender is also "best-effort, non-production" and **only delivers to pre-authorized (team) addresses** — so real external users get no auth email at all. NOT billing/usage (invoices paid, URL allow-list correct, account healthy — all ruled out live).
- **"Loop" UX amplifier (separate, code).** Supabase returns the failure in the URL **hash** (`#error=…otp_expired`), which the **server** route `app/auth/callback/route.ts` cannot read → it falls through to `/forgot-password?error=invalid_link`, and `ForgotPasswordForm` doesn't read `?error`/hash → renders a blank form. So even a legitimately expired link looks like a silent loop.
- **Fix.** **(A) Required, owner/config:** configure **custom SMTP** (Resend/SendGrid/SES/Postmark/Brevo/ZeptoMail) in Authentication → Emails → SMTP (verify `jaanch-ai.in` sending domain), then raise the now-unlocked email rate limit (default 30/h after SMTP). Fixes reset + magic-link + signup-confirm together. **(B) Optional, code (FE):** client-side handler on `/forgot-password` to read the hash/`?error` and show "link expired / too many requests — request a new one" instead of the silent blank form. **(C) Durable, code:** switch recovery to a 6-digit OTP code (`verifyOtp`) — immune to one-time-link consumption by email scanners and works cross-device (no PKCE verifier needed). Files: `frontend/src/components/features/auth/{ForgotPasswordForm,ResetPasswordForm}.tsx`, `frontend/src/app/auth/callback/route.ts`.
- **Interim:** Google sign-in is unaffected (doesn't touch the email bucket).
