...
**GAP-26 (P2, OPEN): citation verifier marks mis-resolved Companies-1956 sections `verified` against the 2013 Act via partial section-match** | Status: OPEN (surfaced 2026-06-04 by the GAP-24 fix making retrieval reachable)
- **Symptom.** Citations to *Companies Act 1956* sections `205A`/`205`/`205(C)`/`205A(8)`/`206A` (unpaid-dividend regime) are `verified` against the **2013** Act doc (`4f2a53e4`), whose `205` is a *different* provision (company-secretary functions). All are `section_only` (no quoted_text), so finding *any* `~205` section = `verified` at 100%.
- **Two pre-existing causes, both OUT of GAP-24's retrieval scope.** (a) **Loose section match** — `get_section_chunks` partial-match ([act_indexer.py:387](backend/app/engines/citation/act_indexer.py#L387)) lets `205A` match index `205` (verdict semantics, deliberately untouched by GAP-24). (b) **Act mis-resolution** — the act-resolver pointed 1956-Act citations at the 2013 doc. Invisible while *everything* failed; GAP-24 exposed it.
- **Why not bundled.** GAP-24 fixed RETRIEVAL only (could not change verdict semantics per its scope). This needs its own pass: tighten partial-match for `NNN`-vs-`NNNX` and/or fix the resolver's vintage targeting. Don't "fix" by reverting retrieval.
