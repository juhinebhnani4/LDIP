...
**What shipped:** one conftest wall (`backend/tests/conftest.py`) — autouse structlog cache-off + contextvars reset, autouse real-httpx-egress block, a canonical `authed_client` via `app.dependency_overrides[get_current_user]`, and a `mock_request` (real `starlette.Request` + scoped limiter disable). Quarantine trimmed **218 → 186** (removed 32 that now pass for the right reason: chunking_logging 16, test_users auth 9, summary get_verifications 2, timeline/dpp002 state-pollution greens 5). Final CI-parity run (`--cov`, placeholder env, empty GCP): **2872 passed / 0 failed / 0 xpassed / 186 xfailed**. Required `test` job stays green.
