...
- **Verified.** Unit: `_section_core` 9/9 cases + match table (`205A`↛`205`, `205(C)`↛`205`, `39A`→`39A`, `21(1)`→`21`, `138`→`138(1)`). Live read-only: watchman=40. All dependent modules import; script compiles. **Pending:** worker+API deploy → run data-fix `--apply` → re-confirm 213/226→0 and GAP-24 set unchanged. hostile-review pass complete (churn-trap, counter-honesty, field-boundary bug caught: `act_name` is post-resolution and masked the mismatch → fixed to read `act_name_original`).
