...
**Total bugs**: 121 | **Fixed**: 75 | **Open**: 37 | **Partially Fixed**: 2 | **Not Reproducible**: 2 | **Not a Bug**: 2 | **Mitigated**: 2
...
- **Follow-up risks — RISK-3 & RISK-2 FIXED (2026-06-03), RISK-1 OPEN (next pass):**
  ...
  - **RISK-1 (OPEN, pre-existing & ARCH-003/ARCH-007-shaped):** citation verification fires from transient chain callbacks on first completion, so it can run on partial embeddings or be silently skipped for reconciler-healed Acts (the reconciler dispatches embed-only with no callbacks). Proper fix is to **derive "act needs verification for matter X" from persisted state** (`act_resolutions` + `library_documents.status` + `library_chunks` NULL-count + `citations.verification_status`) and fire it from a reconciler exactly once the Act is truly fully embedded — likely by completing the existing `sync_citation_statuses_with_resolutions` reconciler. Verification is idempotent (`get_citations_for_act(exclude_verified=True)`), so reconciler-fired re-verify converges to a no-op.
