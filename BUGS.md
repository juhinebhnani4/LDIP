...
**PROD-004 (P2): background readers don't respect the matter soft-delete window — "zombie" reads** | Status: PART B FIXED+DEPLOYED+VERIFIED 2026-06-10 (commit 0c46f77; LDIP 51120d03 + ldip-worker 003d1519 both SUCCESS; /health/invariants live clean, counts unchanged, 0 deleted-matter ids in samples). Part A deferred → PROD-005.
...
- **Part A (stamp the matter's documents on delete) — DEFERRED into PROD-005.** It needs PROD-005's constraint fix to use the full `deleted_at`+`status='deleted'` convention; doing it before that would write a half-convention (`deleted_at` only) and require a second touch. Folded into PROD-005 so it lands once, correctly. Low urgency (docs unreachable via UI; CASCADE reclaims at 30d).
...
**PROD-005 (P2): `soft_delete_document` writes `status='deleted'`, REJECTED by the live CHECK constraint → single-document delete 500s** | Status: OPEN (found 2026-06-10 during PROD-004 architecture-guard)
- **Evidence.** `documents_status_check` allows only `{pending, processing, ocr_complete, ocr_failed, pending_review, chunking, chunking_failed, embedding, embedding_failed, searchable, completed, failed}` — **no `'deleted'`**. `document_service.soft_delete_document` (`document_service.py:663-677`) updates `{deleted_at, status:'deleted'}` atomically → Postgres rejects with 23514 → the generic `except` re-raises as `DocumentServiceError` (500). The cascade hard-delete runs *after* the update, so it never fires (citations/chunks preserved, no data loss — but the document is NOT deleted). Live: **0 documents** have `status='deleted'` or `deleted_at` set anywhere in the DB, consistent with the write never succeeding.
- **Fix direction (own scoped pass).** Add `'deleted'` to the `documents.status` CHECK (one-line additive migration) so the existing INF-009 convention actually works; then **fold in PROD-004 Part A** — `delete_matter` stamps its documents `deleted_at`+`status='deleted'` using the same convention. Needs its own blast-radius (who reads `documents.status` expecting the 12-value set?) + architecture-guard (touches `documents.status`). Related: feedback-soft-delete-status memory, INF-009.
