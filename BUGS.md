...
**GAP-28 (P1): citation EXTRACTION corrupts section + act_name (the master root under GAP-26)** | Status: **L2 FIXED (2026-06-10, code; pending deploy + backfill)** — found by a live E2E upload; blast-radius + choose-solution (L2) + architecture-guard (clean)
...
- **Verified.** Regex no longer bleeds the suffix; `canonicalize_section` 6/6; repair fixes the exact capstone (`205`+`"A of the Companies Act"` → `205A`+`"Companies Act"`; `205(C)`→`205C`); full chain imports. **Pending:** deploy worker; one-off **backfill** of the 146 corrupted-and-verified rows (re-derive section from raw → reset changed ones to pending → re-verify; churn-safe like the GAP-26 fix). **Deferred L3** (regex = span-detection only, LLM owns structure) if the repair keeps having to fix a high rate of regex output. Under-extraction (8 vs 149) is a SEPARATE open item (Gemini `max_output_tokens=8192` truncation, no retry). See `docs/PROD-FINDINGS-2026-06-10.md`.
