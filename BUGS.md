...
**FE-024 (P3): `npm run lint` is not green — 2 errors + 52 warnings, pre-existing** | Status: PARTIALLY FIXED (2026-06-03) — 2 blocking errors fixed; 52 warnings remain
...
- **Still open**: the 52 warnings (esp. the 5 `exhaustive-deps` — worth a real look for stale-closure bugs). No cheap lint guard exists for the transient-flag shape itself; the guard is soft (`useTransientValue` is the obvious primitive so new flags use it).
