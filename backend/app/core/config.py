...
class Settings(BaseSettings):
    ...
    gemini_generation_timeout: float = (
        50.0  # Hard timeout for Gemini generation calls (seconds)
    )

    # GPT-4 Configuration (Story 5-2: Contradiction Detection)
    openai_comparison_model: str = (
        "gpt-4o"  # Was gpt-4-turbo-preview; gpt-4o is 75% cheaper
    )
    ...
    contradiction_screening_model: str = (
        "gemini-2.5-flash"  # Fast/cheap for initial screening
    )
    ...
    contradiction_escalate_results: list[str] = [
        "contradiction",
        "needs_review",
    ...
    entity_extraction_use_batch: bool = (
        True  # Use mega-batch extraction (5 chunks/call)
    )
    ...
    gemini_min_request_delay: float = (
        0.05  # Min seconds between requests (burst dampener, Phase 3: 0.2→0.05)
    )
    gemini_requests_per_minute: int = (
        1000  # Target RPM (Paid Tier 1, conservative vs 4K limit)
    )
    gemini_requests_per_day: int = (
        150000  # Daily quota (Paid Tier 1: 150K RPD for 3.1 Flash Lite)
    )
    ...
    verification_threshold_optional: float = (
        90.0  # > 90% confidence = optional verification
    )
    verification_threshold_suggested: float = (
        70.0  # > 70% confidence = suggested verification
    )
    ...
    orchestrator_weight_contradiction: float = (
        1.2  # Contradiction engine (slightly higher)
    )
    ...
    rag_rerank_top_n: int = (
        6  # Results to return after reranking (fallback when no QueryProfile)
    )
    ...
    query_history_max_entries: int = (
        500  # Max query history entries per matter (JSONB limit)
    )
    query_history_default_limit: int = (
        100  # Default entries returned from get_query_history
    )
    ...
    job_stale_timeout_minutes: int = (
        30  # Jobs in PROCESSING for > this are considered stale
    )
    ...
    admin_emails: str = (
        ""  # Comma-separated list of admin emails (ADMIN_EMAILS env var)
    )
    ...
    table_detection_confidence_threshold: float = (
        0.70  # Min confidence to include table
    )

    # Layout-Aware Chunking Configuration (Docling Integration)
    layout_aware_chunking_enabled: bool = (
        True  # Use Docling layout detection for chunking
    )
    ...
    auto_evaluation_enabled: bool = (
        False  # Auto-evaluate after ingestion (cost warning)
    )
    openai_evaluation_model: str = (
        "gpt-4o"  # Model for RAGAS evaluation (needs structured output support)
    )
    ...
    evaluation_schedule_enabled: bool = (
        False  # Nightly batch evaluation (cost warning: ~$0.15-0.30/matter/run)
    )
    ...
    validation_garbage_base_confidence: float = (
        0.5  # Base confidence for garbage detection
    )
    ...
    email_notifications_enabled: bool = (
        True  # Global feature flag for email notifications
    )
    ...
    contradiction_cache_enabled: bool = (
        True  # CONTRADICTION_CACHE_ENABLED=false to disable
    )
    contradiction_cache_ttl: int = 172800  # 48 hours in seconds
    contradiction_prompt_version: str = (
        "v1"  # Bump to invalidate cache on prompt changes
    )
    ...
    date_extraction_batching_enabled: bool = (
        True  # DATE_EXTRACTION_BATCHING_ENABLED=false to disable
    )

    # F1: Shared chunk loading (cache chunks in Redis after chunking)
    shared_chunk_cache_enabled: bool = (
        True  # SHARED_CHUNK_CACHE_ENABLED=false to disable
    )
