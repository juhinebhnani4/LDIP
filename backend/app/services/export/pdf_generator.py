...
Story 4.3: Court-Ready Certification (Epic 4 - Legal Defensibility)
...
if TYPE_CHECKING:
    ...
    from app.services.export.court_certification import ExportCertificate
...
class PDFGenerator:
    ...
    def generate(
        ...
        certification: ExportCertificate | None = None,
    ) -> bytes:
        ...
        Args:
            ...
            certification: Optional court-ready certification (Story 4.3).
        ...
        pdf_content = self._build_pdf(
            matter_name, section_content, verification_summary, certification
        )
    ...
    def _build_pdf(
        ...
        # Story 4.3: Add certification page first if provided
        if certification:
            self._add_certification_page(text_lines, certification)
            text_lines.append("")
    ...
    def _add_certification_page(
        self, lines: list[str], certification: ExportCertificate
    ) -> None:
        """Add court-ready certification page.

        Story 4.3: AC 4.3.3 - PDF exports include certification page.

        Args:
            lines: List of text lines to append to.
            certification: Export certificate data.
        """
        lines.append("*" * 50)
        lines.append("CERTIFICATE OF AUTHENTICITY")
        lines.append("*" * 50)
        lines.append("")

        # Certification statement
        lines.append(certification.certification_statement)
        lines.append("")

        # Document integrity
        lines.append("-" * 40)
        lines.append("DOCUMENT INTEGRITY")
        lines.append("-" * 40)
        lines.append(f"Content Hash ({certification.hash_algorithm}):")
        lines.append(f"  {certification.content_hash}")
        lines.append("")
        ...
        lines.append("-" * 40)
        lines.append(f"Generated At: {certification.generated_at.strftime('%Y-%m-%d %H:%M:%S %Z')}")
        lines.append(f"Generated By: {certification.generated_by_name}")
        lines.append(f"Email: {certification.generated_by_email}")
        lines.append(f"Role: {certification.generated_by_role}")
        lines.append(f"Tool Version: {certification.tool_version}")
        lines.append("")
        ...
        lines.append("-" * 40)
        lines.append(f"Matter ID: {certification.matter_id}")
        lines.append(f"Matter Name: {certification.matter_name}")
        lines.append("")

        # Verification status
        vs = certification.verification_status
        lines.append("-" * 40)
        lines.append("VERIFICATION STATUS AT EXPORT")
        lines.append("-" * 40)
        lines.append(f"Total Findings: {vs.total_findings}")
        lines.append(f"Verified: {vs.verified_count}")
        lines.append(f"Pending: {vs.pending_count}")
        lines.append(f"Rejected: {vs.rejected_count}")
        lines.append(f"Flagged: {vs.flagged_count}")
        lines.append(f"Verification Rate: {vs.verification_rate:.1f}%")
        lines.append("")

        # Model versions
        lines.append("-" * 40)
        lines.append("AI MODEL VERSIONS USED")
        lines.append("-" * 40)
        for model in certification.models_used:
            lines.append(f"- {model.engine_name}:")
