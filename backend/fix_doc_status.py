...
def main():
    ...
    # Get completed jobs
    jobs = client.table("processing_jobs").select("document_id").eq("status", "COMPLETED").execute()
    ...
    docs = client.table("documents").select("id, filename, status").in_("id", completed_doc_ids).eq("status", "processing").execute()

    if not docs.data:
        print("No documents need fixing!")
        return

    print(f"Found {len(docs.data)} documents with status='processing' but COMPLETED jobs:\n")
