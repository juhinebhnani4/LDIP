...
def main():
    ...
    response = (
        client.table("processing_jobs").select("*").eq("status", "PROCESSING").execute()
    )
    ...
    for j in all_stuck:
        ...
        print(
            f"  - {j['id'][:8]}... | {j['status']} | {stage} | {pct}% | recovery: {rec}"
        )
    ...
    print(
        "  cd backend && .venv\\Scripts\\celery -A app.workers.celery worker --pool=threads --concurrency=4 -l INFO"
    )
