...
def diagnose_matter(matter_id: str | None = None):
    ...
    print(
        f"   Citations on page 1: {page1_count} ({page1_count / total_count * 100:.1f}%)"
        if total_count > 0
        else "   No citations"
    )
    print(
        f"   Citations on other pages: {other_pages} ({other_pages / total_count * 100:.1f}%)"
        if total_count > 0
        else ""
    )
    ...
    print(
        f"   Citations WITH bbox links: {has_bbox_count} ({has_bbox_count / total_count * 100:.1f}%)"
        if total_count > 0
        else "   No citations"
    )
    print(
        f"   Citations WITHOUT bbox links: {empty_bbox_count} ({empty_bbox_count / total_count * 100:.1f}%)"
        if total_count > 0
        else ""
    )
    ...
    if empty_bbox_count > total_count * 0.3 and total_count > 10:
        print(
            "   [!] WARNING: Many citations missing bbox links - highlights won't work!"
        )
    ...
    if doc_ids:
        ...
        print(
            f"   Chunks WITH page_number: {chunks_with_page} ({chunks_with_page / total_chunks * 100:.1f}%)"
            if total_chunks > 0
            else "   No chunks"
        )
        print(
            f"   Chunks WITHOUT page_number: {chunks_without_page} ({chunks_without_page / total_chunks * 100:.1f}%)"
            if total_chunks > 0
            else ""
        )
        ...
        if chunks_without_page > chunks_with_page:
            print(
                "   [!] WARNING: Many chunks missing page_number - this causes citation page issues!"
            )
    ...
    if problem_resp.data:
        for i, citation in enumerate(problem_resp.data, 1):
            ...
            print(
                f"      Page: {citation['source_page']}, BBoxes: {len(citation.get('source_bbox_ids') or [])}"
            )
    else:
    ...
    if page1_count > total_count * 0.3 and total_count > 10:
        ...
        print(
            f"   python scripts/backfill_citation_pages.py --matter-id {matter_id} --dry-run"
        )
        needs_backfill = True
    ...
    if empty_bbox_count > total_count * 0.3 and total_count > 10:
        ...
        print(
            f"   python scripts/backfill_citation_source_bboxes.py --matter-id {matter_id} --dry-run"
        )
        needs_backfill = True

    if (
        chunks_without_page > chunks_with_page
        if "chunks_without_page" in dir()
        else False
    ):
        print("\n3. Run chunk page backfill first:")
        print(
            f"   python scripts/backfill_chunk_page_numbers.py --matter-id {matter_id}"
        )
...
def main():
    ...
    parser.add_argument(
        "--list-matters", action="store_true", help="List available matters"
    )
