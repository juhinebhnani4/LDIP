...
def count_events(client, matter_id: str, include_manual: bool = True) -> dict:
    ...
    query = (
        client.table("events")
        .select("id, is_manual", count="exact")
...
def delete_extracted_events(
    client, matter_id: str, keep_manual: bool = True, dry_run: bool = False
) -> int:
...
def extract_events_from_chunks(
    chunks: list[dict], matter_id: str, batch_size: int = 10, dry_run: bool = False
) -> dict:
...
def main():
    parser = argparse.ArgumentParser(
        description="Re-extract timeline events with improved prompts"
    )
    ...
    parser.add_argument(
        "--keep-manual", action="store_true", help="Keep manually added events"
    )
    ...
    deleted = delete_extracted_events(
        client, args.matter_id, keep_manual=args.keep_manual, dry_run=args.dry_run
    )
    ...
    for event_type, count in sorted(
        result["events_by_type"].items(), key=lambda x: -x[1]
    ):
    ...
    if args.dry_run:
        ...
        try:
            ...
            if entities:
                ...
                for i in range(0, len(events), batch_size):
                    ...
                    print(
                        f"  Batch {i // batch_size + 1}: linked {len(event_entities)} events"
                    )
        ...
        print(
            "WARNING: This requires Celery worker to be running. Use --sync-linking for guaranteed execution."
        )
        try:
            ...
            task1 = classify_events_for_matter.delay(
                matter_id=args.matter_id, force_reclassify=True
            )
