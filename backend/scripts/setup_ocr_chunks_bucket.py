#!/usr/bin/env python3
"""Setup Supabase Storage bucket for OCR chunk results.

This script ensures the 'ocr-chunks' storage bucket exists with proper configuration.
Used for storing cached OCR results from chunked large PDF processing.

Usage:
    python scripts/setup_ocr_chunks_bucket.py
...
def setup_ocr_chunks_bucket() -> bool:
    """Create the ocr-chunks storage bucket if it doesn't exist.
...
def main() -> int:
    """Main entry point."""
    print("=" * 60)
    print("LDIP OCR Chunks Bucket Setup")
    print("=" * 60)
    ...
    success = setup_ocr_chunks_bucket()
