...
def test_helper_function():
    ...
    result = _find_citation_page_from_bboxes(
        "Section 205(C) of the Companies Act", bboxes
    )
    ...
    result = _find_citation_page_from_bboxes(
        "provisions of Companies Act regulations", bboxes
    )
...
def test_accuracy_on_real_data(matter_id: str | None = None, sample_size: int = 50):
    ...
    print(
        f"Same page (agree):      {results['same_page']} ({results['same_page'] / results['total'] * 100:.1f}%)"
    )
    print(
        f"Different page:         {results['different_page']} ({results['different_page'] / results['total'] * 100:.1f}%)"
    )
    print(
        f"New method no match:    {results['new_no_match']} ({results['new_no_match'] / results['total'] * 100:.1f}%)"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Test citation page detection accuracy"
    )
    ...
    parser.add_argument(
        "--sample-size", type=int, default=50, help="Number of citations to test"
    )
    parser.add_argument(
