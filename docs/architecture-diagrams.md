...
graph TB
    ...
    subgraph Railway["Railway Cloud"]
        subgraph API["API Service (FastAPI)"]
            ROUTES[HTTP Routes<br/>39 endpoints]
...
graph TB
    subgraph API_LAYER["API Layer (app/api/)"]
        direction TB
        ROUTES_GROUP["Routes (39 endpoints)"]
        DEPS["deps.py<br/>Auth + DI"]
        WS_GROUP["WebSocket<br/>connection_manager / redis_bridge / auth"]
    end

    subgraph CORE_LAYER["Core Layer (app/core/)"]
        direction TB
        CONFIG["config.py<br/>Settings + Env"]
        SECURITY["security.py<br/>JWT / Auth"]
        COST["cost_tracking.py<br/>LLM Cost Calc"]
        CIRCUIT["circuit_breaker.py"]
        RATE["rate_limit.py"]
        BBOX_SEARCH["bbox_search.py<br/>Spatial Search"]
        OCR_CLEAN["ocr_cleaner.py"]
        GEMINI["gemini_client.py"]
        LLM_RATE["llm_rate_limiter.py"]
    end

    subgraph SERVICE_LAYER["Services Layer (app/services/)"]
        direction TB
        subgraph DATA_SERVICES["Data Access"]
            DOC_SVC["document_service"]
            CHUNK_SVC["chunk_service"]
            CITATION_SVC["citation_service"]
            MATTER_SVC["matter_service"]
            TIMELINE_SVC["timeline_service"]
            BBOX_SVC["bounding_box_service"]
            SUMMARY_SVC["summary_service"]
        end
        subgraph SUB_SERVICES["Specialized Services"]
            RAG_SVC["rag/<br/>embedder, reranker,<br/>hybrid_search, pipeline"]
            CHUNK_PROC["chunking/<br/>text_splitter,<br/>parent_child_chunker,<br/>bbox_linker"]
            OCR_SVC["ocr/<br/>processor, confidence,<br/>pattern_corrector,<br/>bbox_extractor"]
            MIG_SVC["mig/<br/>extractor, resolver,<br/>graph, aliases"]
            CONTRA_SVC["contradiction/<br/>comparator,<br/>statement_query"]
            SAFETY_SVC["safety/<br/>guardrail, language_police,<br/>injection_detector"]
            EXPORT_SVC["export/<br/>pdf, docx, pptx,<br/>court_certification"]
            EVAL_SVC["evaluation/<br/>ragas, ab_testing,<br/>regression_detector"]
            MEM_SVC["memory/<br/>redis_client, query_cache,<br/>session, matter_memory"]
            JOB_SVC["job_tracking/<br/>tracker, progress,<br/>time_estimator"]
            TABLE_SVC["table_extraction/<br/>docling_provider,<br/>layout_extractor"]
        end
    end

    subgraph ENGINE_LAYER["Engines Layer (app/engines/)"]
        direction TB
        CITATION_ENG["citation/<br/>extractor, discovery,<br/>verifier, india_code"]
        CONTRA_ENG["contradiction/<br/>classifier, comparator,<br/>scorer"]
        TIMELINE_ENG["timeline/<br/>date_extractor, builder,<br/>anomaly_detector"]
        RAG_ENG["rag/<br/>generator, query_profile"]
        SUMMARY_ENG["summary/<br/>prompts"]
        ORCH_ENG["orchestrator/<br/>intent_analyzer, planner,<br/>executor, aggregator"]
    end

    subgraph WORKER_LAYER["Workers Layer (app/workers/)"]
        direction TB
        ...
        W_MAINT["maintenance_tasks<br/>quota_monitoring<br/>reasoning_archive"]
...
graph LR
    ...
    TABLE["extract_tables<br/>(Docling)"]
...
graph TB
    ...
    subgraph MATTER_GROUP["(matter) Group — /[matterId]"]
    ...
    OTHER_ROUTES["Other: /settings, /inspector, /auth/callback"]
...
```mermaid
graph TB
    subgraph ZUSTAND["Zustand Stores (18+)"]
        MS["matterStore<br/>current matter + workspace"]
        CS["chatStore<br/>Q&A conversations"]
        WS_STORE["workspaceStore<br/>active tab"]
        US["uploadStore<br/>upload wizard state"]
        PS["processingStore<br/>job tracking"]
        VS["verificationStore<br/>findings queue"]
        NS["notificationStore<br/>alerts"]
        QAS["qaPanelStore<br/>panel position/size"]
        SPLITS["splitViewStore<br/>pdfSplitViewStore"]
        OTHERS["modelStore, libraryStore,<br/>featureStore, inspectorStore,<br/>activityStore, backgroundProcessingStore"]
    end

    subgraph HOOKS["Custom Hooks (55)"]
        AUTH_HOOK["useAuth / useSession"]
        DATA_HOOKS["useMatterSummary, useCitations,<br/>useEntities, useTimeline,<br/>useContradictions, useDocuments"]
        REALTIME["useSSE / useWebSocket"]
        ADMIN_HOOKS["useAdminStatus, useLLMQuota,<br/>useQueueStatus, useUsageDashboard"]
        UI_HOOKS["useSplitView, useBoundingBoxes,<br/>useCrossTabNavigation"]
    end

    subgraph API_CLIENT["API Client (lib/api/)"]
        CLIENT["client.ts<br/>Token injection + refresh<br/>30s timeout + error handling"]
        MODULES["36 API modules<br/>chat, citations, documents,<br/>entities, matters, search,<br/>timeline, verifications, ..."]
    end
...
graph TB
    ...
    subgraph RERANK["Reranking"]
        RERANKER["Voyage / Cohere Reranker<br/>reranker_factory"]
    end
    ...
    USER_Q --> QR --> QP
    QP --> SEMANTIC & BM25 & LIBRARY_SEARCH
    SEMANTIC & BM25 & LIBRARY_SEARCH --> HYBRID
    HYBRID --> RERANKER
    RERANKER --> CONTEXT --> SAFETY --> GEN --> STREAM
...
graph TB
    ...
    subgraph LAYER4["Layer 4: Content Safety"]
        SAFETY2["injection_detector — SQL/prompt injection<br/>safety_guard + language_police — content filtering<br/>quote_detector — detect manipulated quotes"]
    end
